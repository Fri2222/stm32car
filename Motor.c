...
/**
  ...
  * 参    数：Speed 要设置的速度，范围：-100~100
  * 返 回 值：无
	*注意事项 ：In1设置为0，In2设置为1
  */
void Motor_Left_Back_SetSpeed(int8_t Speed)
{
	if (Speed >= 0)							//如果设置正转的速度值
	{
		GPIO_ResetBits(Motor_Left_Pin_1_Port, Motor_Left_Pin_1);	//置低电平
		GPIO_SetBits(Motor_Left_Pin_2_Port, Motor_Left_Pin_2);	//置高电平，设置方向为反转
		PWM_1_SetCompare1(Speed);				//PWM设置为速度值
	}
	else									//否则，即设置反转的速度值
	{
		GPIO_SetBits(Motor_Left_Pin_1_Port, Motor_Left_Pin_1);	//置低电平
		GPIO_ResetBits(Motor_Left_Pin_2_Port, Motor_Left_Pin_2);	//置高电平，设置方向为反转
		PWM_1_SetCompare1(-Speed);			//PWM设置为负的速度值，因为此时速度值为负数，而PWM只能给正数
	}
}
/**
...
void Motor_Right_Back_SetSpeed(int8_t Speed)
{
	if (Speed >= 0)							//如果设置正转的速度值
	{
		GPIO_SetBits(Motor_Right_Pin_1_Port, Motor_Right_Pin_1);	//置低电平
		GPIO_ResetBits(Motor_Right_Pin_2_Port, Motor_Right_Pin_2);	//置高电平，设置方向为反转
		PWM_2_SetCompare1(Speed);				//PWM设置为速度值
	}
	else									//否则，即设置反转的速度值
	{
		GPIO_ResetBits(Motor_Right_Pin_1_Port, Motor_Right_Pin_1);	//置低电平
		GPIO_SetBits(Motor_Right_Pin_2_Port, Motor_Right_Pin_2);	//置高电平，设置方向为反转
		PWM_2_SetCompare1(-Speed);			//PWM设置为负的速度值，因为此时速度值为负数，而PWM只能给正数
	}
}
/**
